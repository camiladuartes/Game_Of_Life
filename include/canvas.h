    ...
    class Canvas
    {
        ...
        public:
            ...
            void pixel( const Point2& point,  const Color& color){
                //RECEBE COORDENADA E A COR QUE QUER COLOCAR NO PIXEL(4 canais)
                for(int y = point.y*m_block_size; y < point.y*m_block_size + m_block_size; y++){
                    for(int x = point.x*m_block_size; x < point.x*m_block_size + m_block_size; x++){
                        m_pixels[y * m_width*image_depth + x*image_depth +0] = color.channels[0]; //255;
                        m_pixels[y * m_width*image_depth + x*image_depth +1] = color.channels[1]; //0;
                        m_pixels[y * m_width*image_depth + x*image_depth +2] = color.channels[2]; //0;
